...
- `create` / `mkdir` / `symlink` / `link` / `unlink`, same-directory `rename` and the parent side of `rmdir` check the name against the lease's in-memory table, apply the change to it, update the parent inode in-core and queue a changelog record. `create` / `mkdir` / `symlink` cache the child inode unpublished and return without a PUT; a rename that replaces a name drops the victim inode synchronously.
- `lookup` / `readdir` are served from the table.
- A flusher thread PUTs unpublished child inodes, then appends queued records in batches (`POST …/append` with the lease token, then a CAS `PUT` of meta) and writes the parent's mtime/nlink once per batch. An append that lands past the committed `log_bytes` (a peer's refused append left garbage) or a log over 1 MiB triggers compaction under the log/snap locks through `/txn`. The inode PUT-before-Link order means a name is never visible without its inode object.
