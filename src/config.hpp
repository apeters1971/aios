...
struct Config {
  ...
  // Session threads for the HTTP front-end. Sessions block for their whole
  // keep-alive lifetime, so this is the cap on concurrent HTTP clients.
  // 0 picks 4 per core, clamped to [8, 64].
  int http_workers{0};
  // Idle timeout for HTTP session reads. A client that opens a connection and
